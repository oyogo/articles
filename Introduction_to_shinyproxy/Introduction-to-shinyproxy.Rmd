...
# Issues encountered:  
1. locating the shinyproxy.jar file : mine was in the /opt/shinyproxy/
2. Ensure that the command to run your application on the application.yml file is as it is in the dockerfile :
   The line:
          container-cmd: ["R","-e","shiny::runAPP('/path/to/your/App')"] should match 
3. Start up configurations : port 2375
4. using simple authentication rather than ldap (on the application.yml file)
5. permission denied : user cannot talk to docker given that the user is non-root
 The docker daemon binds to a Unix socket instead of a TCP port. By default that Unix socket is owned by the root and other users can only access it using sudo. as such shinyproxy is not able to talk to docker hence the error permission denied!
 How to go about it: 
    
      $ sudo groupadd docker # creating a docker group if it doesn't exist
      $ sudo usermod -aG docker $USER # Adding user to the docker group
      Log out and log back in so that the group membership of the user is re-evaluated. 
