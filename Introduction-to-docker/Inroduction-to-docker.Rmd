...
This repository contains an introductory guide to getting started with docker.   
...
![container_vs_vm](www/container-vm.png)  
...
# Building the image.  
Once we've written our dockerfile as explained above navigate to the working directory containing you application files and the following command to build the image:  

![](www/dbuild.png)

docker build is the command for building docker image from dockerfile.   
The -t tags our image as in giving our image a name.   
The . instructs docker to use the dockerfile in the current folder.  
...
You will have to wait for some minutes for your image to be built depending on the specs of your computer and internet speed.  
...
Remember our application used data and if you were keen we did not add a volume in our docker file
